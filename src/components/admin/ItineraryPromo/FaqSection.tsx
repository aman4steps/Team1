...
const FaqSection = () => {
    const [ExclusionGroup, setExclusionGroup] = useState({ groupName: "", description: "" });
    ...
    const handleDelete = async () => {
        if (deleteGroupId) {
            try {
                const response = await fetch('/api/package/tour-info/exclusion/delete', {
                    method: 'POST',
    ...
    const handleEdit = (id) => {
        ...
        setExclusionGroup({ groupName: group.groupName, description: group.description });
    ...
    const handleSave = async () => {
        const url = isEditing
            ? '/api/package/tour-info/exclusion/edit'
            : '/api/package/tour-info/exclusion/add';
        const payload = isEditing
            ? { group_id: editGroupId, ...ExclusionGroup }
            : ExclusionGroup;
        ...
        try {
            ...
            if (response.ok) {
                fetchGroups();
                setExclusionGroup({ groupName: '', description: '' });
                setIsEditing(false);
                setEditGroupId(null);
            } else {
                    ...
                    <div className='flex flex-col'>
                        ...
                        <div className="flex items-center gap-5">
                            <input
                                ...
                                type="text"
                                value={ExclusionGroup.groupName}
                                onChange={(e) => setExclusionGroup({ ...ExclusionGroup, groupName: e.target.value })}
                        ...
                        {/* Display the currently selected group name */}
                            ...
                            <div className='w-full  h-[225px]'>
                                <QuillNoSSRWrapper
                                    ...
                                    theme="snow"
                                    value={ExclusionGroup.description}
                                    onChange={(html) => setExclusionGroup({ ...ExclusionGroup, description: html })}
                                    modules={modules}
                                />
                            </div>
                        </div>
                    ):(
                        <div className="min-w-40 flex h-full justify-center bg-slate-100 rounded-md  items-center ">
                            <div className="flex flex-col items-center justify-center self-center">
                                <BiSolidBookContent className="text-slate-300" size={36}/>
                                <p className=" text-md text-slate-300">Group Select</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50">
                    ...
                    <div className="bg-white p-8 rounded shadow-lg z-50">
                        ...
                        <div className="flex justify-end mt-4">
                            <button onClick={() => toggleModal(null)} className="mr-4 bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                                Cancel
                            </button>
