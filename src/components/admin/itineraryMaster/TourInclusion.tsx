...
const TourInclusion = () => {
    const [inclusionGroup, setInclusionGroup] = useState({ groupName: "", description: "" });
    ...
    const handleDelete = async () => {
        if (deleteGroupId) {
            try {
                const response = await fetch('/api/package/tour-info/inclusion/delete', {
                    method: 'POST',
    ...
    const handleEdit = (id) => {
        ...
        setInclusionGroup({ groupName: group.groupName, description: group.description });
    ...
    const handleSave = async () => {
        const url = isEditing
            ? '/api/package/tour-info/inclusion/edit'
            : '/api/package/tour-info/inclusion/add';
        const payload = isEditing
            ? { group_id: editGroupId, ...inclusionGroup }
            : inclusionGroup;
        ...
        try {
            ...
            if (response.ok) {
                fetchGroups();
                setInclusionGroup({ groupName: '', description: '' });
                setIsEditing(false);
                setEditGroupId(null);
            } else {
                    ...
                    <div className='flex flex-col'>
                        ...
                        <div className="flex items-center gap-5">
                            <input
                                ...
                                type="text"
                                value={inclusionGroup.groupName}
