...
const PaymentTermComponent = () => {
    const [PaymentTermGroup, setPaymentTermGroup] = useState({ groupName: "", description: "" });
    ...
    const handleEdit = (id) => {
        ...
        setPaymentTermGroup({ groupName: group.groupName, description: group.description });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                fetchGroups();
                setPaymentTermGroup({ groupName: '', description: '' });
                setIsEditing(false);
                setEditGroupId(null);
            } else {
            ...
            <div className="md:flex gap-5">
                ...
                <div className=" grow border rounded p-4 h-72">
                    <div className='flex flex-col'>
                        <label className="pb-2 font-semibold">Tour PaymentTerm</label>
                        <div className="flex items-center gap-5">
                            <input
                                className='border rounded-md h-8 px-2 grow focus:border-primary outline-none'
                                type="text"
                                value={PaymentTermGroup.groupName}
                                onChange={(e) => setPaymentTermGroup({ ...PaymentTermGroup, groupName: e.target.value })}
                        ...
                        {/* Display the currently selected group name */}
                            ...
                            <div className='w-full  h-[225px]'>
                                <QuillNoSSRWrapper
                                    ...
                                    theme="snow"
                                    value={PaymentTermGroup.description}
                                    onChange={(html) => setPaymentTermGroup({ ...PaymentTermGroup, description: html })}
                                    modules={modules}
                                />
                            </div>
                        </div>
...
export default PaymentTermComponent;
