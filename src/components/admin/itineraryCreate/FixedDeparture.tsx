...
const FixedDeparture = ({ itinerary, setActiveTab }) => {
    ...
    const [rate, setRate] = useState(itinerary?.prices?.rate || 0);
    const [inventory, setInventory] = useState(itinerary?.prices?.inventory || 0);
    const [weightOptional, setWeightOptional] = useState(itinerary?.prices?.weightOptional || 0);
    
    // State for Age Policy
    const [agePolicy, setAgePolicy] = useState(itinerary?.agePolicy || []);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await fetch('/api/package/info/' + itinerary.id, {
                method: 'POST',
                headers: {
