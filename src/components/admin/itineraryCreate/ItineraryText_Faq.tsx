...
export default function ItineraryText_Faq({ setActiveTab, itinerary }) {
    ...
    const [itineraryDayWise, setItineraryDayWise] = useState({ title: "", information: "" });
    ...
    const ItineraryTextPost = async () => {
        ...
        try {
            const response = await fetch(`/api/package/day-wise/${itinerary?._id}`, {
                ...
                body: JSON.stringify({ days: itineraryDayWiseDataArray, information: itineraryText })
            });
            if (response.ok) {
                setActiveTab("Tab4");
            } else {
    ...
    return (
        <>
            <div>
                <div>
                    <div className="mb-5 flex flex-col">
                        <label htmlFor="itineraryText" className="font-semibold text-base">Day Wise Information</label>
                        <textarea value={itineraryText} onChange={(e) => handleItineraryValidate(e.target.value)} className="resize-none w-full h-30 outline-none border rounded-md px-3 py-2"
                            placeholder="Days Wise Information" rows={5}></textarea>
                        <div>
                            <span className="text-xs text-red-700 capitalize">{itineraryValidate}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="border p-3 rounded">
                        <div className="justify-between gap-5">
                            <div className='border p-4 rounded-md bg-white grow'>
                                <div>
                                    <label htmlFor="title" className="pb-2 font-semibold text-para">Title</label>
                                    <div className='grow flex gap-5 items-center'>
                                        <input className='border rounded-md h-8 px-2 text-para grow focus:border-primary outline-none'
                                            type="text" id="title" name="title" value={itineraryDayWise.title}
                                ...
                                <div className='mt-5'>
                                    ...
                                    <div className="pt-12">
                                        <button onClick={addItem} className={`bg-navyblack text-white rounded px-10 py-1`}>
                                        ...
                                        {itineraryDayWiseDataArray?.map((item, index) => (
                                            <li key={index}>
                                                <div className='flex justify-between even:bg-slate-50 px-1'>
                                                    <div>
                                                        <p className='capitalize flex gap-2'><span>Day {index + 1}. </span>{item.title}</p>
                                                    </div>
                                                    <div className='flex gap-2'>
                                                        <FaEdit onClick={() => editItem(index)} size={20} className='mt-1 hover:text-primary cursor-pointer' />
                                                        {editingIndex !== index && (
                                                            <MdDeleteForever onClick={() => removeItem(index)} size={24} className='mt-1 hover:text-red-500 cursor-pointer' />
                                                        )}
                                                    </div>
                                                </div>
                                            </li>
            ...
            <div>
                <button onClick={ItineraryTextPost} className="bg-black text-white w-full rounded py-2">Save</button>
            </div>
        </>
    )
}
